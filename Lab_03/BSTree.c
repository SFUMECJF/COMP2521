...
void BSTreeLevelOrder(BSTree t)
{
	...
	// while the queue still has some entries
	while (!QueueIsEmpty(q)) {

		// remove the node at the front of the queue and print the value
		t =  QueueDequeue(q);
		printf("%d ", t->value);
	
		// traverse through the tree
		BSTree left = t->left;
		BSTree right = t->right;

		// add left child (if any) to queue
		if (left != NULL) {
			QueueEnqueue(q, left);
		}

		// add right child (if any) to queue
		if (right != NULL) {
...
int BSTreeNumLeaves(BSTree t)
{
	...
	// base case - reached a leaf if there are no children of the node
	if (t->left == NULL && t->right == NULL) {
		return 1;
	}

	// recursive case - travserse through the tree
